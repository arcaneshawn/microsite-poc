...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://sliceymachinetest.cdn.prismic.io/api/v2', Object.assign({}, options,  {"routes":[{"type":"pagev2","path":"/:uid"},{"type":"subpage","path":"/:parentpage/:uid","resolvers":{"parentpage":"parentpage"}}]}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://sliceymachinetest.cdn.prismic.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady(() => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=sliceymachinetest&new=true'
